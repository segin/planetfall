...
void init_direction(ZObjectID id, const char *name, const char *syn1,
                    const char *syn2) {
...
void setup_terminal() {
  ...
  // Set Scroll Region (Lines 2 to Height)
...
void update_status_bar() {
  ...
  // Save Cursor
  printf("\033[s");
  // Move to Home (1,1) - Output for Status Bar
  printf("\033[H");
...
int main(int argc, char **argv) {
  ...
  legacy_dispatch:
    ZObjectID prso =
        (current_cmd.prso_count > 0) ? current_cmd.prso_list[0] : NOTHING;
    ...
    case V_BOARD:
      if (prso == NOTHING) {
        ...
        } else if ((prso == O_GLOBAL_POD || prso == R_ESCAPE_POD) &&
                   (current_room == R_DECK_NINE || current_room == R_CRAG)) {
    ...
    case V_OPEN:
      if (prso == NOTHING) {
        ...
        } else if (obj_has_flag(prso, F_DOORBIT) ||
                   obj_has_flag(prso, F_CONTBIT)) {
          obj_set_flag(prso, F_OPENBIT);
          tellf("Opened.\n");
          if (prso == O_POD_DOOR && current_room == R_ESCAPE_POD) {
            if (game_state.trip_counter > 0) {
              tellf("The bulkhead opens and cold ocean water rushes in! You "
                    "drown.\n");
            }
          }
        } else {
        ...
        if (!obj_has_flag(prso, F_OPENBIT)) {
          ...
          if (prso == O_POD_DOOR && current_room == R_ESCAPE_POD) {
            if (!is_event_enabled(EVT_POD_TRIP)) {
              ...
              tellf("\nThe pod door clangs shut and the pod begins to "
                    "vibrate.\n");
            }
          }
        } else {
    ...
    case V_REMOVE:
      if (prso == O_PATROL_UNIFORM) {
        if (obj_in(O_PATROL_UNIFORM, player) &&
          ...
          if (obj_in(O_BLATHER, current_room)) {
            tellf("\n'Removing your uniform while on duty? Five hundred "
                  "demerits!'\n");
          }
        } else {
    ...
    case V_WALK: {
      ...
      case O_NORTH:
        ...
        break;
      case O_SOUTH:
        dest = objects[current_room].south;
        break;
      case O_EAST:
        dest = objects[current_room].east;
        break;
      case O_WEST:
        dest = objects[current_room].west;
        break;
      case O_NE:
        dest = objects[current_room].ne;
        break;
      case O_NW:
        dest = objects[current_room].nw;
        break;
      case O_SE:
        dest = objects[current_room].se;
        break;
      case O_SW:
        dest = objects[current_room].sw;
        break;
      case O_UP:
        dest = objects[current_room].up;
        break;
      case O_DOWN:
        dest = objects[current_room].down;
        break;
      case O_IN:
        dest = objects[current_room].in;
        break;
      case O_OUT:
        dest = objects[current_room].out;
        break;
      default:
